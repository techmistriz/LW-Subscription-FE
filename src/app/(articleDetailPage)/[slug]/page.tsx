...
import { getArticleBySlug, getRelatedPosts } from "@/lib/api/posts";
import { stripInlineStyles, toTitleCase } from "@/lib/utils";
  ...
  async function fetchRelated() {
    ...
    try {
      ...
      if (article.category_id) {
        requests.push(
          getRelatedPosts({ category_id: article.category_id })
        );
      }

      if (article.author_id) {
        requests.push(
          getRelatedPosts({ author_id: article.author_id })
        );
      }

      if (article.magazine_id) {
        requests.push(
          getRelatedPosts({ magazine_id: article.magazine_id })
        );
      }
